...
clustering_data <- filtered_p_value_data %>%
  ...
  summarise(log_p_value = mean(log_p_value, na.rm = TRUE),
            mouse_strain = first(mouse_strain),  # Using first strain as a proxy for all strains
            mouse_life_stage = first(mouse_life_stage),  # Similarly for life stage
...
ui <- fluidPage(
  titlePanel("Hierarchical Clustering of Gene_Symbols Based on normalised log p-value"),
  sidebarLayout(
    sidebarPanel(
      ...
      selectizeInput("selected_genes", label = "Select gene_symbols:",
                     ...
                     multiple = TRUE,
                     options = list(create = TRUE, maxItems = 150, placeholder = 'Start typing a gene symbol')),
