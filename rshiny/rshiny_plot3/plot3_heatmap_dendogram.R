...
ui <- fluidPage(
  titlePanel("Hierarchical Clustering of Gene_Symbols Based on Z-Score (normalised log p-value)"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output, session) {
  ...
  reactive_heatmap <- reactive({
    ...
    # Normalize log_p_values in matrix into Z-scores 
    clustering_matrix <- scale(clustering_matrix)
    
    # Rename the column to Z-score (normalized log p-value)
    colnames(clustering_matrix) <- "Z-score (normalized log p-value)"
