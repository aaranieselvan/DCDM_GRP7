...
```
#1 Create the tables which contain the raw experimental analysis data
...
#Load data from clean_final_data.csv into the mouse_clean table
...
#Add procedure_id to the table
alter table procedure_info
add procedure_id VARCHAR(100);
```

```{sql connection=}
#Create parameter_description table
...
#Add data from disease_info and mice_info into mouse_disease_relationship
INSERT INTO mouse_disease_relationship (disease_term, disease_id, gene_accession_id, phenodigm_score, analysis_id, gene_symbol)
SELECT disease_info.disease_term, disease_info.disease_id, disease_info.gene_accession_id, disease_info.phenodigm_score, mice_info.analysis_id, mice_info.gene_symbol
FROM disease_info
LEFT JOIN mice_info
ON disease_info.gene_accession_id = mice_info.gene_accession_id;
```
...
#Add data from parameter_description and mice_info into mouse_parameter_relationship
INSERT INTO mouse_parameter_relationship (impcParameterOrigId, parameter_id, name, description, analysis_id, gene_symbol)
SELECT parameter_description.impcParameterOrigId, parameter_description.parameter_id, parameter_description.name, parameter_description.description, mice_info.analysis_id, mice_info.gene_symbol
FROM parameter_description
LEFT JOIN mice_info
ON parameter_description.parameter_id = mice_info.parameter_id;
...
#Add data from procedure_info and parameter_description into parameter_procedure_relationship
INSERT INTO parameter_procedure_relationship (procedure_id, procedure_name, procedure_description, parameter_id, impcParameterOrigId)
SELECT procedure_info.procedure_id, procedure_info.name,procedure_info.description, parameter_description.parameter_id, parameter_description.impcParameterOrigId 
FROM procedure_info
LEFT JOIN parameter_description
ON parameter_description.impcParameterOrigId = procedure_info.impcParameterOrigId;
...
INSERT INTO mouse_procedure (procedure_id, procedure_name, parameter_id, parameter_description_name, gene_symbol, impcParameterOrigId)
SELECT parameter_procedure_relationship.procedure_id, parameter_procedure_relationship.procedure_name, mouse_parameter_relationship.parameter_id, mouse_parameter_relationship.name, mouse_parameter_relationship.gene_symbol, mouse_parameter_relationship.impcParameterOrigId
FROM parameter_procedure_relationship
LEFT JOIN mouse_parameter_relationship
ON mouse_parameter_relationship.impcParameterOrigId = parameter_procedure_relationship.impcParameterOrigId;
